...
interface TableRow {
  ...
  kota?: string;
  location: string;
  data: { [key: string]: { [fieldName: string]: string } }; // year -> fieldName -> value
}

export default function DataTable({ data }: DataTableProps) {
  // Group data by provinsi and kota separately
  const groupedData = useMemo(() => {
    const groups: { [key: string]: { provinsi: string; kota: string | null; items: IndikatorDetailData[] } } = {};
  ...
  // Get unique years and fieldNames for column headers
          ...
          <thead className="bg-gray-50 dark:bg-gray-900">
            <tr>
              ...
              {uniqueYears.map(year => (
                <th key={year} colSpan={uniqueFieldNames.length} className="px-6 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider border-r border-gray-200 dark:border-gray-700">
                  {year}
                </th>
              ))}
            </tr>
            {uniqueFieldNames.length > 1 && (
              <tr>
                ...
                {uniqueYears.map(year => 
                  uniqueFieldNames.map((fieldName, index) => (
                    <th key={`${year}-${fieldName}`} className="px-3 py-2 text-center text-xs font-medium text-gray-500 dark:text-gray-400 border-r border-gray-200 dark:border-gray-700">
                      <div 
              ...
              <tr key={key} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                ...
                {uniqueYears.map(year => 
                  uniqueFieldNames.map(fieldName => {
                    const item = group.items.find(i => i.tahun === year && i.fieldName === fieldName);
                    return (
                      <td key={`${year}-${fieldName}`} className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white text-center border-r border-gray-200 dark:border-gray-700">
                        {item ? `${item.dataValue}` : '-'}
                      </td>
                    );
                  })
